...
for epoch in range(nr_epochs):
    ...
    for x, y in train_loader:
        optimizer.zero_grad()
        output, weights = hypernet(x, return_weights=True, discretize=True)
...
chosen_indices = np.random.choice(range(X_train.shape[0]), 1, replace=False)
chosen_examples = X_train[chosen_indices]
_, weights, tree = hypernet(torch.tensor(chosen_examples).float().to('cpu'), return_weights=True, discretize=True, return_tree=True)
...
example = chosen_examples[0]
first_feature = example[0]
second_feature = example[1]
feature_indices = tree[0]
feature_indices = torch.squeeze(feature_indices, 0)
feature_indices = torch.squeeze(feature_indices, 1)
feature_indices = feature_indices.detach().cpu().numpy()
feature_thresholds = tree[1]
feature_thresholds = torch.squeeze(feature_thresholds, 0)
feature_thresholds = feature_thresholds.detach().cpu().numpy()
classes = tree[2]
leaf_classes = []
...
ax[0, 1].scatter(first_feature, second_feature, color=colors[index], marker=markers[index], label=labels[index],s=35)
